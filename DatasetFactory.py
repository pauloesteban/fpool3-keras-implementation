...
def load_wav_files_and_assign_their_labels(wav_files_directory, label_for_files_found_in_directory):
    ...
    for wav_file in wav_files_directory.iterdir():
        if wav_file == Path("dataset/training/positive-identification/.gitignore") \
            or wav_file == Path("dataset/evaluation/positive-identification/.gitignore"):
            continue
...
def load_training_data_and_its_labels():
    ...
    x_data = np.reshape(x_data, [418, 40, 32, 1])  # Tensor: number of wav files * number of mfcc * t * 1
